...
const updatePaginationDiv = (currentPage, numPages) => {
  ...
  const endRange = Math.min(currentPage * PAGE_SIZE, totalPokemonCount);
  $('#pokemonCount').text(`Showing ${startRange} - ${endRange} of ${totalPokemonCount} Pokémon`);
};
...
const paginate = async (currentPage, PAGE_SIZE, pokemons) => {
    ...
    const startRange = (currentPage - 1) * PAGE_SIZE + 1;
    const endRange = Math.min(currentPage * PAGE_SIZE, totalPokemonCount);
    $('#displayedPokemonCount').text(`Showing ${startRange} - ${endRange} of ${totalPokemonCount} Pokémon`);
};
